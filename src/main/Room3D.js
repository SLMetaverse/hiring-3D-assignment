...
export default class Room3D {
    constructor(room) {
        ...
        this.#createWall(0xffffff, 0, 100, 0, Math.PI/2, 0, false) //ceilling
        this.#createWall(0xffffff, 0, 0, 0, -Math.PI/2, 0, false) //floor
        if (!walls.some(e => e === Walls.FRONT)) this.#createWall(0x7f7fff, 0, 50, -50, 0, 0, true)
        if (!walls.some(e => e === Walls.BACK))  this.#createWall(0xff7fff, 0, 50, 50, 0, 0, true)
        if (!walls.some(e => e === Walls.RIGHT)) this.#createWall(0x00ff00, 50, 50, 0, 0, -Math.PI/2, true)
        if (!walls.some(e => e === Walls.LEFT))  this.#createWall(0xff0000, -50, 50, 0, 0, Math.PI/2, true)
    ...
    #createWall(color, x, y, z, rotationX, rotationY, isCollider) {
        ...
        if (isCollider) Colliders.add(planeTop)
    }
}